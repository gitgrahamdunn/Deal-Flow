...
SELECT
    ...
    w.lat,
    w.lon,
    (printf('%02d-%02d-%03d-%02dW%d', CAST(w.lsd AS INTEGER), w.section, w.township, w.range, w.meridian)) AS area_key,
    r.last_prod_month,
    CAST(
        (
            (CAST(strftime('%Y', r.as_of_date) AS INTEGER) - CAST(strftime('%Y', COALESCE(r.last_prod_month, p.last_prod_month)) AS INTEGER)) * 12
            + (CAST(strftime('%m', r.as_of_date) AS INTEGER) - CAST(strftime('%m', COALESCE(r.last_prod_month, p.last_prod_month)) AS INTEGER))
        ) AS INTEGER
