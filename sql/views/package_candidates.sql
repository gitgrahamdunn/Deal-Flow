...
SELECT
    ...
    ROUND(COALESCE(SUM(lfp.trailing_oil_bbl), 0), 2) AS linked_facility_trailing_oil_bbl,
    ROUND(COALESCE(SUM(lfp.trailing_gas_mcf), 0), 2) AS linked_facility_trailing_gas_mcf,
    ROUND(
        MIN(
            100,
            cw.cluster_score * 0.55
            + COUNT(DISTINCT bwf.facility_id) * 4
            + SUM(CASE WHEN df.facility_type = 'BATTERY' THEN 1 ELSE 0 END) * 3
            + COUNT(DISTINCT CASE WHEN bwf.facility_id IS NOT NULL THEN cw.well_id END) * 1.5
            + CASE
                WHEN cw.suspended_well_count > 0
                THEN (COUNT(DISTINCT CASE WHEN bwf.facility_id IS NOT NULL THEN cw.well_id END) * 1.0 / cw.suspended_well_count) * 12
                ELSE 0
              END
        ),
        2
