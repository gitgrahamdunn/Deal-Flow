...
SELECT
    ...
    ROUND(ot.total_estimated_restart_upside_bpd, 2) AS total_estimated_restart_upside_bpd,
    ot.total_linked_facilities,
    ot.total_facility_linked_wells,
    ROUND(ot.avg_package_score, 2) AS avg_package_score,
    ROUND(ot.top_package_score, 2) AS top_package_score,
    ...
    ROUND(
        MIN(
            ...
            + CASE WHEN ot.package_count = 1 THEN 12 ELSE 0 END
            + CASE
                WHEN (
                    CASE
                        WHEN COALESCE(ot.total_suspended_wells, 0) > 0
                ...
                ELSE 0
              END
            - CASE WHEN ot.package_count >= 4 THEN 8 ELSE 0 END
        ),
        2
    ) AS footprint_score
